...
#include "SmartROSBridgeOpcUaBackendPortFactory.hh"
...
static SmartROSBridgeOpcUaBackendPortFactory OpcUaBackendPortFactory;

SmartROSBridgeOpcUaBackendPortFactory::SmartROSBridgeOpcUaBackendPortFactory()
{  
	componentImpl = 0;
	SmartROSBridge::instance()->addPortFactory("OpcUa_SeRoNet", this);
}

SmartROSBridgeOpcUaBackendPortFactory::~SmartROSBridgeOpcUaBackendPortFactory()
{  }

void SmartROSBridgeOpcUaBackendPortFactory::initialize(SmartROSBridge *component, int argc, char* argv[])
{
	componentImpl = new SeRoNet::Utils::Component(component->connections.component.name);
}

int SmartROSBridgeOpcUaBackendPortFactory::onStartup()
{
	if (!component_thread.joinable()) {
    	component_thread = std::thread(&SmartROSBridgeOpcUaBackendPortFactory::task_execution, this);
    	return 0;
    }
	return -1;
}
...
int SmartROSBridgeOpcUaBackendPortFactory::task_execution()
{
	componentImpl->run();
	return 0;
}

int SmartROSBridgeOpcUaBackendPortFactory::onShutdown(const std::chrono::steady_clock::duration &timeoutTime)
{
...
void SmartROSBridgeOpcUaBackendPortFactory::destroy()
{
