...
TestCore::TestCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
void TestCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const Test* test = dynamic_cast<const Test*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(test);
		}
	}
}

void TestCore::attach_interaction_observer(TestObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void TestCore::detach_interaction_observer(TestObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int TestCore::execute_protected_region()
{
...
void TestCore::updateAllCommObjects()
{
...
void TestCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int TestCore::getPreviousCommObjId()
{
